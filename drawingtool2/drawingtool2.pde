...
void setup(){
  ...
  for (int i=0; i<formResolution; i++){
    ...
    z[i] = (sin(angle*i)+ cos(angle*i)) * initRadius;  
...
void draw(){
  ...
  if (mouseX != 0 || mouseY != 0) {
    ...
    centerZ += ((mouseY-centerY)+(mouseX-centerX)) * 0.01;
  ...
  if (mode == 0) {
    beginShape();
    // start controlpoint
    curveVertex(x[formResolution-1]+centerX, y[formResolution-1]+centerY, z[formResolution-1]+centerY);

    // only these points are drawn
    for (int i=0; i<formResolution; i++){
      curveVertex(x[i]+centerX, y[i]+centerY, z[i]+centerY);
    }
    curveVertex(x[0]+centerX, y[0]+centerY, z[0]+centerY);

    // end controlpoint
    curveVertex(x[1]+centerX, y[1]+centerY, z[1]+centerY);
    endShape();
  }
